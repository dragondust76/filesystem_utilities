...
# Get a list of block devices (disks and partitions)
lsblk -o NAME,FSTYPE,SIZE,MOUNTPOINT -P | while read -r line; do
    ...
    if [[ "$FSTYPE" == "" ]]; then
        # This is likely a disk, print in a different color
        echo -e "\e[1m\e[33mDisk: $NAME (Size: $SIZE)\e[0m"
    else
        # This is a partition, print with filesystem type and mount point
